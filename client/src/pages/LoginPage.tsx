...
export default function LoginPage() {
  ...
  // Fetch server config to know if Google OAuth is available
  useEffect(() => {
    fetch(`${API_BASE}/auth/config`)
      .then(r => r.ok ? r.json() : null)
      .then(data => { if (data?.googleOAuthEnabled) setGoogleOAuthEnabled(true); })
      .catch(() => {});
  }, []);

  // Handle token passed back from Google OAuth callback (web) or deep link (native)
  ...
  // On native (Capacitor), listen for the deep-link that carries the OAuth token
  useEffect(() => {
    if (!Capacitor.isNativePlatform()) return;
    const subscription = CapApp.addListener('appUrlOpen', async ({ url }) => {
      // Expected format: shiftsync://login?token=<JWT> or shiftsync://login?error=<msg>
      try {
        ...
        if (token) {
          await Browser.close();
          loginWithToken(token).catch(() => setError('Google sign-in failed.'));
        } else if (err) {
          await Browser.close();
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 sm:px-8 py-10 lg:py-0">
        ...
        <div className="w-full max-w-[400px] space-y-5">
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm p-6 sm:p-8 space-y-5">
            ...
            {tab === 'login' ? (
              <>
                {/* Google Sign-In — only rendered when server has Google OAuth configured */}
                {googleOAuthEnabled && (
                  <>
                    <button
                      type="button"
