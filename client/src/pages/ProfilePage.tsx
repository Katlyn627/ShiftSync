        ...
        <>
          ...
          <Card className="p-5">
            <div className="flex items-start gap-4">
              <div className={`w-14 h-14 rounded-full flex items-center justify-center text-lg font-bold shrink-0 ${AVATAR_BG[myEmployee.role] ?? 'bg-muted text-muted-foreground'}`}>
              ...
              <div className="mt-3 space-y-3">
                {colleagues.map(col => (
                  <div key={col.id} className="flex items-center gap-3 p-3 rounded-lg bg-muted/30 border border-border">
                    <div className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${AVATAR_BG[col.role] ?? 'bg-muted text-muted-foreground'}`}>
                      {initials(col.name)}
                    </div>
                    <div className="flex-1 min-w-0">
