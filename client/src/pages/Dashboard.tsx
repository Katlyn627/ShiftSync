  ...
  if (schedules.length === 0) {
    return (
      <div className="text-center py-20">
        <p className="text-muted-foreground text-lg">No schedules yet.</p>
        <p className="text-muted-foreground/70 mt-1">Go to the Schedule tab to generate one.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <select
          className="border border-border rounded-md px-3 py-1.5 text-sm text-foreground bg-input-background focus:outline-none focus-visible:ring-2 focus-visible:ring-ring"
          value={selectedId ?? ''}
      ...
      {isManager && staffingChartData.length > 0 && (
        <Card className="p-5">
          <h2 className="text-lg font-semibold mb-1 text-foreground">📊 Demand-Based Staffing Suggestions</h2>
          <p className="text-xs text-muted-foreground mb-4">Recommended staff count per day based on forecast revenue</p>
          <div className="grid grid-cols-7 gap-2 text-center text-xs">
            {staffingSuggestions.map(day => {
              ...
              return (
                <div key={day.date} className="bg-muted/30 rounded-lg p-2 border border-border">
                  <div className="font-semibold text-muted-foreground">{DAY_NAMES[day.day_of_week]}</div>
                  <div className="text-muted-foreground/70">{day.date.slice(5)}</div>
                  <div className="text-lg font-bold text-primary mt-1">{totalStaff}</div>
                  <div className="text-muted-foreground/70">staff</div>
                  {day.expected_revenue > 0 && (
                    <div className="text-muted-foreground mt-1">${(day.expected_revenue / 1000).toFixed(1)}k</div>
                  )}
                  <div className="mt-1 space-y-0.5">
                    {Object.entries(roleGroups).map(([role, count]) => (
                      <div key={role} className="text-muted-foreground">{count} {role}</div>
                    ))}
                  </div>
                </div>
              );
      ...
      {isManager && laborCost && laborCost.by_day.length > 0 && (
        <Card className="p-5">
          <h2 className="text-lg font-semibold mb-4 text-foreground">Daily Labor Cost</h2>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {isManager && laborCost && laborCost.by_role.length > 0 && (
          <Card className="p-5">
            <h2 className="text-lg font-semibold mb-4 text-foreground">Cost by Role</h2>
            <div className="space-y-2">
              {laborCost.by_role.map(r => (
                <div key={r.role} className="flex items-center gap-2">
                  <span className="text-sm w-24 text-muted-foreground">{r.role}</span>
                  ...
                  <span className="text-sm font-medium text-foreground w-16 text-right">${r.cost.toFixed(0)}</span>
                </div>
              ))}
            </div>
          </Card>
        ...
        <Card className="p-5">
          <h2 className="text-lg font-semibold mb-4 text-foreground">🔥 Burnout Risk Monitor</h2>
          {burnout.length === 0 ? (
            <p className="text-muted-foreground/70 text-sm">No data yet</p>
          ) : (
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {burnout.map(b => (
                <div key={b.employee_id} className="flex items-start gap-3 p-2 rounded-lg bg-muted/30">
                  <span
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-muted-foreground">{b.weekly_hours}h/wk</span>
                    </div>
                    {b.factors.length > 0 && (
...
function KpiCard({ label, value, sub, color }: { label: string; value: string; sub: string; color: string }) {
  ...
  return (
    <Card className={`${colorClasses[color] ?? colorClasses.green} p-5 border`}>
